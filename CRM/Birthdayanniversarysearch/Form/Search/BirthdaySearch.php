...
class CRM_Birthdayanniversarysearch_Form_Search_BirthdaySearch  extends  CRM_Contact_Form_Search_Custom_Base
...
*/
    ...
    function templateFile( ) {
       return 'CRM/Birthdayanniversarysearch/Form/Search/BirthdaySearch.tpl';
    }
	...
	if ( $onlyIDs ) {
		...
		$select = "DISTINCT contact_a.id as contact_id, 
		                    CONCAT(monthname(contact_a.birth_date) , ' ',  day(contact_a.birth_date)) as sort_name ,year(contact_a.birth_date) as Year_name,
		                    contact_a.display_name as name, 'birthday' as oc_type,
		                    DATE_FORMAT( FROM_DAYS( TO_DAYS( NOW( ) ) - TO_DAYS( contact_a.birth_date ) ) , '%Y' ) +0 AS oc_age " ;
...
//order by month(birth_date), oc_day";
	...
	//for only contact ids ignore order.
      if ( !$onlyIDs ) {
          ...
          if ( ! empty( $sort ) ) {
              ...
              $sql .=   "ORDER BY month(birth_date), day(contact_a.birth_date)";
          }
      }
  ...
  function where($includeContactIDs = false){ 
	...
	//echo "<pre>"; print_r($this->_formValues); echo "</pre>"; exit;
	$clauses = array( ); 
